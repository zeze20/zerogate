...
impl FramePoolModel {
    ...
    ///
    /// Returns `true` if `valid_transition(current, to)` holds and the
    /// transition was applied. Returns `false` on out-of-bounds or
    /// illegal transition (no state change).
    pub fn transition(&mut self, i: usize, to: FrameState) -> bool {
        let Some(&current) = self.states.get(i) else {
            return false;
        };
...
mod tests {
    ...
    fn model_full_rx_cycle() {
        ...
        assert!(pool.transition(0, FrameState::InFill));
        assert!(pool.transition(0, FrameState::Kernel));
        assert!(pool.transition(0, FrameState::Rx));
        assert!(pool.transition(0, FrameState::User));
        assert!(pool.transition(0, FrameState::InFill));
        assert!(pool.transition(0, FrameState::Kernel));
    }
    ...
    fn model_full_tx_cycle() {
        ...
        assert!(pool.transition(0, FrameState::Rx));
        assert!(pool.transition(0, FrameState::User));
        assert!(pool.transition(0, FrameState::Tx));
        assert!(pool.transition(0, FrameState::Completion));
        assert!(pool.transition(0, FrameState::Free));
    ...
    fn model_all_frames_return_to_free() {
        ...
        for i in 0..n {
            assert!(pool.transition(i, FrameState::InFill));
            assert!(pool.transition(i, FrameState::Kernel));
            assert!(pool.transition(i, FrameState::Rx));
            assert!(pool.transition(i, FrameState::User));
            assert!(pool.transition(i, FrameState::Tx));
            assert!(pool.transition(i, FrameState::Completion));
            assert!(pool.transition(i, FrameState::Free));
        }
